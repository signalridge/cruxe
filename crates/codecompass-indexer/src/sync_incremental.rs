...
use codecompass_core::types::{FileRecord, JobStatus};
use codecompass_state::branch_state::{self, BranchState};
use codecompass_state::jobs;
use codecompass_state::tombstones::BranchTombstone;
use codecompass_vcs::{DiffEntry, FileChangeKind, VcsAdapter, WorktreeManager};
...
pub fn apply_tombstones_for_actions(
    ...
    codecompass_state::tombstones::delete_for_ref(conn, repo, ref_name)?;
    ...
    for tombstone in &tombstones {
        codecompass_state::tombstones::create_tombstone(conn, tombstone)?;
    }
    Ok(())
}
    ...
    for action in actions {
        ...
        match action {
            SyncAction::Deleted { .. } => {
                ...
                codecompass_state::manifest::delete_manifest(conn, project_id, ref_name, path)?;
                let source_edge_id = crate::import_extract::source_symbol_id_for_path(path);
                codecompass_state::edges::delete_edges_for_file(
                    conn,
                    project_id,
                    ref_name,
...
mod tests {
    ...
    fn default_worktrees_root_uses_storage_root_not_data_subdir() {
        let tmp = tempdir().unwrap();
        let storage_root = tmp.path().join(".codecompass");
    ...
    fn apply_tombstones_for_actions_replaces_previous_ref_snapshot() {
        ...
        .unwrap();
        let first = codecompass_state::tombstones::list_paths_for_ref(&conn, "repo-1", "feat/auth")
            .unwrap();
    ...
    fn persist_branch_sync_state_writes_branch_state_row() {
        ...
        let row = codecompass_state::branch_state::get_branch_state(&conn, "repo-1", "feat/auth")
            .unwrap()
            .unwrap();
    ...
    fn index_current_checkout_as_base(
        ...
        let base_index_set = codecompass_state::tantivy_index::IndexSet::open(data_dir).unwrap();
        ...
        persist_branch_sync_state(
            ...
            &BranchSyncStateUpdate {
                ...
                overlay_dir: None,
                file_count: codecompass_state::manifest::file_count(conn, project_id, "main")
                    .unwrap() as i64,
                symbol_count: 0,
    ...
    fn manifest_paths(conn: &Connection, repo: &str, ref_name: &str) -> Vec<String> {
        let mut paths: Vec<String> =
            codecompass_state::manifest::get_all_entries(conn, repo, ref_name)
                .unwrap()
                .into_iter()
                .map(|e| e.path)
                .collect();
    ...
    fn run_incremental_sync_releases_lease_and_rolls_back_job_on_plan_error() {
        ...
        let lease =
            codecompass_state::worktree_leases::get_lease(&conn, "proj-vcs", "feat/add-file")
                .unwrap()
    ...
    fn t271_overlay_sync_delete_file_creates_tombstone() {
        ...
        let tombstones = codecompass_state::tombstones::list_paths_for_ref(
            &conn,
            "proj-vcs",
            "feat/delete-file",
        )
