...
impl FastEmbedProvider {
    ...
    fn ensure_runtime(&mut self) {
        ...
        let runtime_init = catch_unwind(AssertUnwindSafe(|| TextEmbedding::try_new(options)));
        match runtime_init {
            Ok(Ok(runtime)) => {
            ...
            Err(panic_payload) => {
                ...
                warn!(
                    model = self.model_id,
                    panic = %panic_reason,
                    "fastembed initialization panicked, falling back to deterministic embeddings"
                );
                if let Ok(mut cache) = runtime_cache().lock() {
                    cache.insert(cache_key, None);
                }
...
impl EmbeddingProvider for ExternalEmbeddingProvider {
    ...
    fn embed_batch(&mut self, inputs: &[String]) -> Result<Vec<Vec<f32>>, StateError> {
        ...
        let api_key = std::env::var("CRUXE_EMBEDDING_API_KEY")
...
fn fastembed_cache_capacity() -> usize {
    std::env::var("CRUXE_FASTEMBED_CACHE_CAP")
        .ok()
...
fn fastembed_runtime_enabled() -> bool {
    parse_fastembed_runtime_flag(
        std::env::var("CRUXE_ENABLE_FASTEMBED_RUNTIME")
            .ok()
