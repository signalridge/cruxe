...
fn build_index_command(request: &IndexLaunchRequest<'_>) -> Command {
    ...
    if let Some(project_id) = request.project_id {
        cmd.env("CODECOMPASS_PROJECT_ID", project_id);
    }
    if let Some(storage_data_dir) = request.storage_data_dir {
        cmd.env("CODECOMPASS_STORAGE_DATA_DIR", storage_data_dir);
    }
    if let Some(job_id) = request.job_id {
        cmd.env("CODECOMPASS_JOB_ID", job_id);
    }
