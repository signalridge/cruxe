...
use codecompass_vcs::{Git2VcsAdapter, WorktreeManager};
use serde::Serialize;
...
pub(super) fn handle_list_refs(params: QueryToolParams<'_>) -> JsonRpcResponse {
    ...
    let project_row = match codecompass_state::project::get_by_id(c, project_id) {
    ...
    let refs = if project_row.vcs_mode {
        match codecompass_state::branch_state::list_branch_states(c, project_id) {
            Ok(mut entries) => {
        ...
        vec![RefDescriptorPayload {
            ...
            file_count: codecompass_state::manifest::file_count(c, project_id, constants::REF_LIVE)
                .unwrap_or(0),
...
pub(super) fn handle_switch_ref(params: QueryToolParams<'_>) -> JsonRpcResponse {
    ...
    let state = match codecompass_state::branch_state::get_branch_state(c, project_id, target_ref) {
        Ok(Some(state)) => state,
        Ok(None) => {
            return tool_error_response(
                id,
    ...
    if let Ok(Some(mut row)) =
        codecompass_state::branch_state::get_branch_state(c, project_id, target_ref)
    {
        row.last_accessed_at = now_iso8601();
        if let Err(err) = codecompass_state::branch_state::upsert_branch_state(c, &row) {
            let (code, message, data) = map_state_error(&err);
            return tool_error_response(id, code, message, data, base_metadata);
        }
    }
