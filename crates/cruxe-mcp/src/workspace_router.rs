use cruxe_core::error::WorkspaceError;
use cruxe_core::types::{WorkspaceConfig, generate_project_id};
...
impl WorkspaceRouter {
    ...
    pub fn resolve_workspace(
        ...
        let conn = cruxe_state::db::open_connection(&self.db_path).map_err(|e| {
        ...
        if let Some(ws) =
            cruxe_state::workspace::get_workspace(&conn, &canonical_str).map_err(|e| {
                WorkspaceError::NotAllowed {
            ...
            })?
        {
            ...
            let _ = cruxe_state::workspace::update_last_used(&conn, &canonical_str, &now);
        ...
        match cruxe_state::workspace::evict_lru_auto_discovered(
            &conn,
        ...
        let should_bootstrap =
            cruxe_state::workspace::claim_bootstrap_indexing(&conn, &canonical_str, &now).map_err(
                |e| WorkspaceError::NotAllowed {
                    path: canonical_str.clone(),
        ...
        if let Ok(Some(ws)) = cruxe_state::workspace::get_workspace(&conn, &canonical_str)
...
mod tests {
    use super::*;
    use cruxe_core::types::AllowedRoots;
    use std::sync::Arc;
    ...
    fn resolve_auto_discovers_workspace_under_allowed_root() {
        ...
        let conn = cruxe_state::db::open_connection(&db_path).unwrap();
        let ws_record = cruxe_state::workspace::get_workspace(&conn, &ws_str).unwrap();
    ...
    fn t239_auto_discover_three_workspaces_updates_known_workspaces() {
        ...
        let conn = cruxe_state::db::open_connection(&db_path).unwrap();
        let workspaces = cruxe_state::workspace::list_workspaces(&conn).unwrap();
    ...
    fn t460_lru_eviction_cleans_evicted_workspace_index_data() {
        ...
        let conn = cruxe_state::db::open_connection(&db_path).unwrap();
        cruxe_state::workspace::update_last_used(&conn, &ws1s, "2026-01-01T00:00:00Z").unwrap();
        cruxe_state::workspace::update_last_used(&conn, &ws2s, "2026-01-02T00:00:00Z").unwrap();
