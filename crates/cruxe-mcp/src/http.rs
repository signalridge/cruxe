...
use cruxe_core::error::ProtocolErrorCode;
use cruxe_core::types::{SchemaStatus, WorkspaceConfig, generate_project_id};
...
fn session_scope_from_headers(headers: &HeaderMap) -> Option<String> {
    headers
        .get("mcp-session-id")
        .or_else(|| headers.get("x-cruxe-session"))
...
mod tests {
    use super::*;
    use cruxe_core::types::{AllowedRoots, Project, WorkspaceConfig};
    ...
    fn session_scope_prefers_mcp_session_id_header() {
        ...
        headers.insert("x-cruxe-session", HeaderValue::from_static("fallback"));
        assert_eq!(
    ...
    fn t230_locate_symbol_http_matches_stdio_shape() {
        ...
        assert_eq!(
            http_payload
                .get("metadata")
                .and_then(|m| m.get("cruxe_protocol_version")),
            stdio_payload
                .get("metadata")
                .and_then(|m| m.get("cruxe_protocol_version"))
        );
    ...
    fn t231_health_reports_indexing_when_active_job_exists() {
        ...
        let conn = cruxe_state::db::open_connection(&db_path).unwrap();
        cruxe_state::schema::create_tables(&conn).unwrap();
        let _ = cruxe_state::tantivy_index::IndexSet::open(&data_dir).unwrap();
