    ...
    match crate::index_launcher::spawn_index_process(&launch_request) {
        Ok(child) => {
            ...
            std::thread::spawn(move || {
                ...
                let mut poll_conn = cruxe_state::db::open_connection(&poll_db_path).ok();
                    ...
                    match child.try_wait() {
                        ...
                        Ok(None) => {
                            if poll_conn.is_none() {
                                poll_conn = cruxe_state::db::open_connection(&poll_db_path).ok();
                            }
                            ...
                            {
                                match cruxe_state::jobs::get_active_job(conn, &poll_project_id) {
                                    Ok(Some(job))
        ...
        Err(e) => tool_error_response(
            ...
            Some(json!({
                "details": e.to_string(),
                "remediation": "Run `cruxe index` manually to inspect logs.",
            })),
            metadata,
        ),
    }
