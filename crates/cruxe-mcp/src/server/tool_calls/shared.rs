...
pub(super) fn resolve_ranking_explain_level(
    arguments: &Value,
    config: &Config,
) -> Result<cruxe_core::types::RankingExplainLevel, String> {
    ...
    if level == cruxe_core::types::RankingExplainLevel::Off && config.debug.ranking_reasons {
        return Ok(cruxe_core::types::RankingExplainLevel::Full);
    }
    Ok(level)
}

fn parse_ranking_explain_level(raw: &str) -> Option<cruxe_core::types::RankingExplainLevel> {
    match raw.trim().to_ascii_lowercase().as_str() {
        "off" => Some(cruxe_core::types::RankingExplainLevel::Off),
        "basic" => Some(cruxe_core::types::RankingExplainLevel::Basic),
        "full" => Some(cruxe_core::types::RankingExplainLevel::Full),
        _ => None,
    }
}

pub(super) fn ranking_reasons_payload(
    reasons: Vec<cruxe_core::types::RankingReasons>,
    level: cruxe_core::types::RankingExplainLevel,
) -> Option<Value> {
    match level {
        cruxe_core::types::RankingExplainLevel::Off => None,
        cruxe_core::types::RankingExplainLevel::Full => serde_json::to_value(reasons).ok(),
        cruxe_core::types::RankingExplainLevel::Basic => {
...
pub(super) fn align_ranking_reasons_to_dedup(
    reasons: &[cruxe_core::types::RankingReasons],
    kept_indices: &[usize],
) -> Vec<cruxe_core::types::RankingReasons> {
...
pub(super) fn map_state_error(err: &StateError) -> (ProtocolErrorCode, String, Option<Value>) {
    match err {
        ...
        StateError::CorruptManifest(details) => (
            ProtocolErrorCode::IndexIncompatible,
            "Index metadata is corrupted. Run `cruxe index --force`.".to_string(),
            Some(json!({
                "details": details,
                "remediation": "cruxe index --force",
            })),
