use super::*;
use cruxe_query::semantic_advisor::{
...
pub(super) fn handle_index_status(params: IndexStatusToolParams<'_>) -> JsonRpcResponse {
    ...
    let (file_count, symbol_count) = conn
        .map(|c| {
            let fc = cruxe_state::manifest::file_count(c, project_id, &effective_ref).unwrap_or(0);
            let sc = cruxe_state::symbols::symbol_count(c, project_id, &effective_ref).unwrap_or(0);
            (fc, sc)
        })
        .unwrap_or((0, 0));
    ...
    let recent_jobs = conn
        .and_then(|c| cruxe_state::jobs::get_recent_jobs(c, project_id, 5).ok())
        .unwrap_or_default();
