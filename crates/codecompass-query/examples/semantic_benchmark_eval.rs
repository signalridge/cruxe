...
use codecompass_query::search::{SearchExecutionOptions, SearchResult, search_code_with_options};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if registered.is_none() {
        return Err(format!(
            "workspace {} is not registered; run `codecompass init` first",
            workspace.display()
        )
        .into());
    }
...
fn print_usage() {
    eprintln!(
        "Usage:
  cargo run -p codecompass-query --example semantic_benchmark_eval -- \\
