...
use codecompass_core::types::RerankResult;
...
fn resolve_rerank_api_key() -> Result<String, StateError> {
    ...
    std::env::var("CODECOMPASS_RERANK_API_KEY")
...
fn shared_rerank_http_client(timeout: Duration) -> Result<Client, StateError> {
    ...
    let cache = RERANK_HTTP_CLIENT_CACHE.get_or_init(|| Mutex::new(HashMap::new()));
    ...
    if let Ok(mut guard) = cache.lock() {
        if guard.len() >= MAX_RERANK_CLIENT_CACHE_ENTRIES
