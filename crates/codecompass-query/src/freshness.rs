use codecompass_core::types::{FreshnessPolicy, FreshnessStatus};
use codecompass_indexer::scanner;
...
pub fn check_freshness_with_scan_params(
    ...
    if let Ok(Some(_)) = codecompass_state::jobs::get_active_job(conn, project_id) {
    ...
    if !codecompass_core::vcs::is_git_repo(workspace) {
...
fn check_single_version_freshness(
    ...
    let Ok(entries) = codecompass_state::manifest::get_all_entries(conn, project_id, r#ref) else {
        return FreshnessResult::Fresh;
    };
    if entries.is_empty() {
        return FreshnessResult::Fresh;
    ...
    // Use the same scanner rules as indexing (gitignore/.codecompassignore + language detection)
