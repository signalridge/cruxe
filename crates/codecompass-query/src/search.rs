use codecompass_core::config::SearchConfig as CoreSearchConfig;
...
pub fn search_code_with_options(
    ...
    let rerank_enabled = options.search_config.semantic_mode_typed()
        != codecompass_core::types::SemanticMode::Off
    ...
    } else if options.search_config.semantic_mode_typed()
        == codecompass_core::types::SemanticMode::Off
    {
...
fn apply_rerank_scores(
    ...
    reranked: &[codecompass_core::types::RerankResult],
) {
...
fn semantic_execution_state(
    ...
    let semantic_skipped_reason = if !semantic_enabled {
        ...
        match semantic_mode {
            codecompass_core::types::SemanticMode::RerankOnly => {
                Some("mode_rerank_only".to_string())
            }
            codecompass_core::types::SemanticMode::Hybrid => None,
            codecompass_core::types::SemanticMode::Off => Some("semantic_disabled".to_string()),
        }
    };

    let rerank_provider = if semantic_mode == codecompass_core::types::SemanticMode::Off {
        "none".to_string()
...
mod tests {
    ...
    use codecompass_state::{db, schema, vector_index, vector_index::VectorRecord};
    ...
    fn hybrid_mode_returns_semantic_match_for_conceptual_query() {
        ...
        let mut provider =
            codecompass_state::embedding::build_embedding_provider(&search_config.semantic)
                .unwrap()
                .provider;
