...
use cruxe_core::vcs;
use cruxe_state::{db, import, maintenance_lock, schema};
use std::path::Path;
...
pub fn run(workspace: &Path, bundle_path: &Path, config_file: Option<&Path>) -> Result<()> {
    ...
    if metadata.parser_version != constants::PARSER_VERSION {
        eprintln!(
            "Warning: imported parser_version={} differs from local parser_version={}. Run `cruxe index --force --path {}` to refresh parser-dependent symbols.",
...
fn remap_imported_project_data(
    ...
    tx.commit().map_err(cruxe_core::error::StateError::sqlite)?;
    Ok(())
}
