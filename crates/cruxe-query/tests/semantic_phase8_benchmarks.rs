...
use cruxe_core::types::{FileRecord, QueryIntent, SnippetRecord, SymbolKind, SymbolRecord};
use cruxe_indexer::writer::BatchWriter;
use cruxe_query::hybrid::semantic_query;
use cruxe_query::search::{SearchExecutionOptions, SearchResponse, search_code_with_options};
...
fn run_phase8_benchmarks() -> Result<Phase8Report, String> {
    ...
    let symbol_case_count = std::env::var("CRUXE_T409_SYMBOL_QUERIES")
        .ok()
    ...
    let buckets = vec![
        (
            "<10k".to_string(),
            read_env_usize("CRUXE_T406_BUCKET_LT10K", 5_000),
        ),
        (
            "10k-50k".to_string(),
            read_env_usize("CRUXE_T406_BUCKET_10K_50K", 20_000),
        ),
        (
            ">50k".to_string(),
            read_env_usize("CRUXE_T406_BUCKET_GT50K", 55_000),
        ),
    ];
...
fn run_natural_language_eval(
    ...
    for case in query_cases {
        ...
        if std::env::var("CRUXE_PHASE8_DEBUG").as_deref() == Ok("1") && case.id == "rust-001" {
            eprintln!(
...
fn write_phase8_report(report: &Phase8Report) -> Result<(), String> {
    let dir = std::env::var("CRUXE_PHASE8_REPORT_DIR")
        .ok()
