...
### User Story 1 - Install Cruxe via Prebuilt Binary (Priority: P1)
...
GitHub releases, run `cruxe --version`, then `cruxe doctor` -- both
must succeed without any additional setup.

**Acceptance Scenarios**:

1. **Given** a macOS arm64 machine without Rust installed, **When** the user
   downloads the release binary and runs `cruxe --version`, **Then** the
...
5. **Given** a Windows x86_64 machine, **When** the user downloads and extracts the
   release archive, **Then** `cruxe.exe` runs without additional DLL
   dependencies.

---

### User Story 2 - Install Cruxe via Homebrew (Priority: P1)

A macOS (or Linux with Homebrew) user installs Cruxe via
`brew install signalridge/tap/cruxe`. The tap is automatically updated
...
**Independent Test**: Run `brew install signalridge/tap/cruxe`, then
`cruxe --version` and `cruxe doctor` -- all must succeed.

**Acceptance Scenarios**:

1. **Given** a macOS user with Homebrew installed, **When** they run
   `brew install signalridge/tap/cruxe`, **Then** the latest release binary
   is downloaded and installed to `$(brew --prefix)/bin/cruxe`.
...
3. **Given** the user runs `brew upgrade cruxe`, **When** a newer version is
...
A developer reads the MCP distribution docs and configures Cruxe as an MCP
...
the agent's config file, start the agent, and verify Cruxe tools are listed
and callable.

**Acceptance Scenarios**:

1. **Given** the Claude Code configuration template, **When** a user adds it to
   their `mcp_servers` config, **Then** Cruxe tools are discoverable via
   `tools/list` and functional.
2. **Given** the Cursor MCP configuration template, **When** a user adds it to
   their Cursor MCP config, **Then** Cruxe tools are discoverable and
   functional.
...
follows the step-by-step instructions to set up Cruxe. The guide includes
...
**Independent Test**: A developer with no prior Cruxe experience follows
...
2. **Given** the integration guide includes a recommended prompt rule
   `"use Cruxe tools before file reads"`, **When** an agent follows this
...
3. **Given** the Cursor integration guide, **When** a user follows the setup
   steps, **Then** Cruxe tools appear in Cursor's MCP tool list.
...
Git hooks trigger `cruxe sync` on commit, and the project stays indexed
...
4. **Given** a git pre-push hook template, **When** installed, **Then** it runs
   `cruxe doctor` before push to verify index health.
...
- What happens when an agent configuration template is outdated?
  ...
  the template version matches their installed Cruxe version.
- What happens when a git hook fails silently?
  The reference hook templates include error handling that logs failures to
  `~/.cruxe/logs/hook.log`. Users are guided to check this log in
  troubleshooting.
- What happens when `cruxe sync` is triggered during an active indexing job?
...
- **FR-804**: System MUST provide a Homebrew tap at `signalridge/tap/cruxe` with
...
> **Cross-reference**: spec 001 US1 also requires `cargo install cruxe` as a
...
- **SC-801**: A user with no Rust toolchain can install and run Cruxe on any of
  the 5 target platforms in under 2 minutes.
- **SC-802**: `brew install signalridge/tap/cruxe` succeeds and the installed
  binary passes `cruxe doctor` on macOS.
- **SC-803**: A new user following any integration guide can have Cruxe working
