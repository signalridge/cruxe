...
| `openspec/**` | traceability drift | Code changes must include matching spec/change updates |
...
1. `cruxe-query/**` relevance tuning + `openspec/specs/semantic-*` updates.
2. `cruxe-state/src/edges.rs` traversal/index tests + `openspec/specs/vcs-*` / `openspec/specs/graph-*`.
3. CI/security workflow updates under `.github/**`.
4. Docs-only updates in `openspec/meta/**` (except migration-critical docs).
