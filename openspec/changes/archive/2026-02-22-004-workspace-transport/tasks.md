...
## 2. Workspace Router (Multi-Workspace Core)
...
## 3. Index Progress Notifications
...
## 5. Security Hardening & Cross-Cutting
...
- [x] T238b Ensure HTTP error mapping uses canonical machine-readable `error.code` values from `openspec/meta/protocol-error-codes.md` (including `invalid_input`, `workspace_not_registered`, `workspace_not_allowed`, `index_incompatible`)
