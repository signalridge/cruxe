...
## 2. Vector Store (Embedded Local Backend, `semantic_mode = hybrid` only)
...
## 3. Embedding Pipeline (Indexing Integration, `semantic_mode = hybrid` only)
...
## 4. Hybrid Search Blending (`semantic_mode = hybrid` only)
...
- [x] T404 [P] Write integration test: call `search_code` via MCP, verify confidence metadata is included in response, and verify semantic/rerank hard-failure paths preserve canonical error envelope and codes from `openspec/meta/protocol-error-codes.md` (non-fail-soft cases only)
