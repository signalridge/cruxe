## 1. Data Layer (Call Edge Storage)
...
## 2. Call Edge Extraction (Tree-Sitter)
...
## 5. Follow-up Query Suggestions (US4)
...
- [x] T357 [P] Update MCP `tools/list` response to include all three new tools with correct schemas, and validate errors for all three tools against `openspec/meta/protocol-error-codes.md` with machine-stable `error.code` values
