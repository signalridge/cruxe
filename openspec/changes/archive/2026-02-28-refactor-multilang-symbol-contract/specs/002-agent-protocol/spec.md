...
| `kind_weight` | 0.5-2.0 | **new** | Tiered by symbol kind (see below) |
| `query_intent_boost` | 0.0-1.0 | **new** | Query naming convention matches kind category |
...
| `test_file_penalty` | -0.5 or 0.0 | **new** | Result from a test file |
...
- **THEN** the breakdown MUST include `kind_match` (replacing the current 0.0 placeholder) with the actual computed `kind_weight + query_intent_boost` value
...
The ranking system MUST apply differentiated boost scores based on symbol kind, replacing the current dead `kind_match = 0.0` placeholder (ranking.rs:29).
...
Field boost values (applied at BM25 retrieval time in Tantivy QueryParser, search.rs:875):
...
Note: snippet search fields (`content`, `path`, `imports`) and file search fields (`path`, `filename`, `content_head`) already include all relevant fields (search.rs:860,864). Only field boost weights are missing — the fields themselves are correctly configured.
