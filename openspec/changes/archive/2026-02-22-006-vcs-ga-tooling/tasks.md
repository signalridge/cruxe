## 1. VCS Analysis Tools (`diff_context`, `find_references`, `explain_ranking`)
...
## 2. Ref Operations Tools (`list_refs`, `switch_ref`)
...
## 3. State Portability (`state export/import`)
...
- [x] T319 Write integration test: start MCP server, call `tools/list`, verify all new tools are listed with correct schemas, and validate new tool errors map to `openspec/meta/protocol-error-codes.md` with stable `error.code` across stdio and HTTP transports

## 5. Overlay Maintenance CLI
...
## 6. Tooling Polish and Verification
...
## 7. Descriptor Stability and Test Throughput
