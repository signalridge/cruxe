...
- [x] 1.1 Update `IndexingStatus` and `ResultCompleteness` in `crates/codecompass-core/src/types.rs` to canonical enum sets required by specs 001-003.
...
- [x] 1.3 Align metadata builder methods in `crates/codecompass-mcp/src/protocol.rs` (`new`, `not_indexed`, `syncing`, `reindex_required`, `corrupt_manifest`) to emit semantically correct canonical `indexing_status` values per the builder mapping table in design.md Decision 2.
...
- [x] 2.1 Add `search.ranking_explain_level` (`off|basic|full`) to `crates/codecompass-core/src/config.rs` and `configs/default.toml`, with normalization logic and environment-variable overrides.
- [x] 2.2 Implement compatibility precedence logic: request arg > config default > legacy `debug.ranking_reasons` fallback.
- [x] 2.3 Extend MCP tool schemas in `crates/codecompass-mcp/src/tools/search_code.rs` and `crates/codecompass-mcp/src/tools/locate_symbol.rs` to accept `compact` and `ranking_explain_level`.
...
- [x] 2.5 Migrate `codecompass-cli` `serve-mcp` entrypoint: remove legacy `enable_ranking_reasons: bool` runtime toggle path and align to `ranking_explain_level` config/env propagation.
...
- [x] 4.1 Add/update unit tests for enum serde, legacy alias compatibility, and ranking explain config parsing in `codecompass-core`.
